...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
  ...
  if (self) {
    ...
    [self.view addSubview:activityView];
      activityView.center = CGPointMake(CGRectGetWidth(self.view.bounds)/2, CGRectGetHeight(self.view.bounds)/2);
    [activityView release];
      
    beepSound = -1;
    decoding = NO;
    OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:CGRectMake(0, 0, 768, 1024)
    ...
    [theOverLayView release];
      ...
      if (version < 7.0)
      {
          [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"nev.png"] forBarMetrics:UIBarMetricsDefault];
      }
  }
...
- (void)setBackItem:(BOOL)white
{
    UIButton *button = [[[UIButton alloc] initWithFrame:CGRectMake(0, 0, 60, 40)] autorelease];
    /*[button setImage:[UIImage imageNamed: white ? @"btn_alerts_back.png" :@"btn_back.png"] forState:UIControlStateNormal];
    [button setImage:[UIImage imageNamed:white ? @"btn_alerts_backh.png" :@"btn_backh.png"] forState:UIControlStateHighlighted];
    [button setTitle:@"返回1" forState:UIControlStateNormal];*/
    
    [button setImage:[UIImage imageNamed:@"ico_return.png"] forState:UIControlStateNormal];
