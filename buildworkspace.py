...
#ipa 文件输出的文件夹
OutPutDir = "/Users/macmini/Documents/archive"
...
def getProjectPath(workSpaceDir):
    for parent,dirnames,filenames in os.walk(workSpaceDir):    #三个参数：分别返回1.父目录 2.所有文件夹名字（不含路径） 3.所有文件名字
        for dir in dirnames:
            if dir.find('xcodeproj',0,len(dir)) != -1:
                return os.path.join(parent,dir)

if __name__ == "__main__":

        parser = argparse.ArgumentParser()
        parser.add_argument("-t","--type",type=int,help='build project type = 0, build workspace type = 1, default = 0',default=0)
        parser.add_argument("-p","--path",help='build path')
        ...
        if args.type == 0:
            if args.path != None:
                ...
                print('请输入xcodeproj文件的路径')

        else:
            if args.path != None:
                workSpacePath = args.path
                dir = os.path.dirname(workSpacePath)
                projectDir = getProjectPath(dir)
                os.chdir(projectDir)
                targets = parseProject(projectDir)
                for target in targets:
                    targetName = target[0]
                    prodoctName = target[1]
                    build_app_command = "xcodebuild -workspace " + workSpacePath + " -scheme " \
                          + targetName + " -archivePath " + os.path.join(OutPutDir,prodoctName+'.xcarchive') + " -configuration Release" + ' archive'
                    result = os.system(build_app_command)
                    if result != 0:
                        log("fail")
                        notify("IPA build fail", prodoctName, "", userInfo={})
